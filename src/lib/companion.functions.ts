...
export const createThread = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row, error } = await context.supabase
      .from("chat_threads")
      .insert({ user_id: context.userId, title: data.title ?? "A quiet conversation" })
      .select().single();
    if (error) throw error;
    return row;
  });
...
export const renameThread = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("chat_threads")
      .update({ title: data.title }).eq("id", data.id).eq("user_id", context.userId);
    if (error) throw error;
    return { ok: true };
  });
...
export const listMemories = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("emotional_memories").select("*")
